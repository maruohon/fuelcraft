...
public class BlockCartPainter extends BlockFuelcraftBase {

    public static final byte YAW_TO_DIRECTION[] = {2, 5, 3, 4};

    public BlockCartPainter(String name, float hardness, Material material)
    {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase livingBase, ItemStack stack)
    {
        ...
        if (te == null || (te instanceof TileEntityCartPainter) == false)
        {
            return;
        }
        ...
        {
            ((TileEntityCartPainter)te).setRotation(YAW_TO_DIRECTION[yaw]);
        }
    }
    ...
    public IIcon getIcon(IBlockAccess blockAccess, int x, int y, int z, int side)
    {
        ...
        if (te instanceof TileEntityCartPainter && side == ((TileEntityCartPainter)te).getRotation())
        {
            return this.icons[2]; // front
        }
    ...
    public void registerBlockIcons(IIconRegister iconRegister)
    {
        ...
        this.icons[2] = iconRegister.registerIcon(ReferenceTextures.getTileName(this.blockName) + ".side");
